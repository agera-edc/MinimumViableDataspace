...
>
> For Example, the `Registration Service` and `EDC` repository hash can be found in the _Checkout_ steps  (in the `ref` property) of [action.yml](../.github/actions/gradle-setup/action.yml):
...
Each `EDC Connector` has its own set of Private and Public keys with java keystore e.g. `system-tests/resources/vault/provider`. These were generated using the following commands:
